...
def get_data_dir() -> Path:
    ...
    Order: TEXT2VIDEO_DATA_DIR env -> platformdirs.user_data_dir("text2video").
    """
    env_path = os.environ.get("TEXT2VIDEO_DATA_DIR")
    if env_path:
        path = Path(env_path).expanduser().resolve()
    else:
        cfg = load_config()
        cfg_path = cfg.get("TEXT2VIDEO_DATA_DIR") if isinstance(cfg, dict) else None
        if cfg_path:
...
def get_outputs_dir() -> Path:
    """Returns the directory for generated videos."""
    env_path = os.environ.get("TEXT2VIDEO_OUTPUT_DIR")
    if env_path:
        path = Path(env_path).expanduser().resolve()
    else:
        cfg = load_config()
        cfg_path = cfg.get("TEXT2VIDEO_OUTPUT_DIR") if isinstance(cfg, dict) else None
        if cfg_path:
            path = Path(cfg_path).expanduser().resolve()
        else:
...
def get_db_path() -> Path:
    ...
    return get_data_dir() / "text2video.db"
...
def ensure_initial_setup():
    ...
    _move_file_if_exists(legacy_root / "text2video.db", db_path)

    config = {
        "version": 1,
        "TEXT2VIDEO_DATA_DIR": None,
        "TEXT2VIDEO_OUTPUT_DIR": None,
        "TEXT2VIDEO_ENABLE_TORCH_COMPILE": None,
    }
    global _CONFIG_CACHE
